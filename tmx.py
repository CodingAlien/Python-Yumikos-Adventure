...
class Tile(object):
    ...
    def loadxml(self, tag):
        ...
        objectgroups = tag.find('objectgroup')
        if not objectgroups is None:
            for og in objectgroups.findall('object'):
                x = og.attrib['x']
                y = og.attrib['y']
                width = og.attrib['width']
                height = og.attrib['height']
                rect = pygame.Rect(float(x), float(y), float(width), float(height))
